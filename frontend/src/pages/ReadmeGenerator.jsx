...
export default function ReadmeGenerator() {
  ...
  if (!form.projectId) {
    toast.error('Select an existing project before saving the README');
    return;
  }

  if (!result.trim()) {
    toast.error('Generate a README first');
    return;
  }
  ...
  try {
    await projectAPI.update(form.projectId, {
      name: form.name,
    ...
    toast.error(error.message || 'Could not save README');
  } finally {
    setSaving(false);
  }
};
          ...
          {!loading && result && (
            <>
              <div className="flex flex-wrap items-center gap-2 border-b border-line px-5 py-3">
                ...
                <Button size="sm" variant="primary" icon="check"  onClick={saveReadme}  isLoading={saving}  disabled={saving}> Save README
                </Button>
              ...
              <div className="flex items-center gap-2 border-t border-line px-5 py-3 text-[12px] text-faint">
                ...
                Generated by the backend Groq endpoint. Review before saving it to a project.
              </div>
            </>
          )}
        </Card>
